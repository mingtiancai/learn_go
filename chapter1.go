...
func HandleServer2(w http.ResponseWriter, r *http.Request) {
	mu.Lock()
	count++
	mu.Unlock()
	fmt.Fprintf(w, "URL.Path = %q\n", r.URL.Path)
...
func Server2() {
	http.HandleFunc("/", HandleServer2)
	http.HandleFunc("/count", counter)
	log.Fatal(http.ListenAndServe("localhost:8000", nil))
}

func HandleServer3(w http.ResponseWriter, r *http.Request) {
	mu.Lock()
	count++
	mu.Unlock()
	fmt.Fprintf(w, "URL.Path = %q\n", r.URL.Path)
...
func Server3() {
	http.HandleFunc("/", HandleServer3)
	http.HandleFunc("/count", counter)
	log.Fatal(http.ListenAndServe("localhost:8000", nil))
}

func Lissajous3(out io.Writer, cycleIn float64) {
	const (
		res     = 0.001
		size    = 100
		nframes = 64
...
func P1_12() {
	...
	if len(os.Args) > 1 && os.Args[1] == "web" {
		...
		handleCycles := func(w http.ResponseWriter, r *http.Request) {
			Lissajous3(w, 20)
		}
		http.HandleFunc("/", handle)
